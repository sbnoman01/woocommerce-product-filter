...
class Class_ShortCode{
        ...
        <div class="swiper-wrapper">
            ...
            <?php
                $args = array(
                    ...
                    'posts_per_page' => 10,        // Retrieve all products (use -1 for all, or a specific number)
                    'post_status' => 'publish',
                    'tax_query'     => [
                        [
                            'taxonomy' => 'item-type',
    ...
    public function wc_product_filter($atts) {
	 ...
	 public function wc_related_product($atts) {
		...
		$atts = shortcode_atts(
			...
			$atts,
			'advanced_related_products'
		);


        $path = plugin_dir_path(__DIR__) . 'public/partials/wc-related-product-display.php';
        
        if( file_exists($path) ){
            require $path;
            return wc_related_product_display($atts);
        }
    }
}